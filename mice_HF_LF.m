...
%Test
p = normcdf((r_lf-r_hf)/sqrt(ste_r_hf.^2 + ste_r_lf.^2)); %
...
%Test
p = normcdf((Gamma_lf(1)-Gamma_hf(1))/sqrt(ste_Gamma_hf(1).^2 + ste_Gamma_lf(1).^2)); %
...
beta_hf3;
ste_beta_hf3;
[rho_hf, pval_hf] = corr(log10(means_hf),log10(vars_hf));
...
beta_lf;
ste_beta_lf;
[rho_lf, pval_lf] = corr(log10(means_lf),log10(vars_lf));

%Tests
p = normcdf((beta_lf-beta_hf)/sqrt(ste_beta_hf.^2 + ste_beta_lf.^2)); %
p = normcdf((beta_nonbacter_lf-beta_nonbacter_hf)/sqrt(ste_beta_nonbacter_hf.^2 + ste_beta_nonbacter_lf.^2)); %


%Hypergeometric tests for phyla-level outliers

%Bacteroidetes LFPP
NN_bacter = length(tax_lf);
KK_bacter = length(bacter_inds_lf);
nn_bacter = length(out_inds_lf);
kk_bacter = length(bacter_out_inds_lf);
pval_hygeo_bacter_lf = hygecdf(kk_bacter,NN_bacter,KK_bacter,nn_bacter,'upper');

%Firmicutes LFPP
NN_firmi = length(tax_lf);
KK_firmi = length(firmi_inds_lf);
nn_firmi = length(out_inds_lf);
kk_firmi = length(firmi_out_inds_lf);
pval_hygeo_firmi_lf = hygecdf(kk_firmi,NN_firmi,KK_firmi,nn_firmi,'upper');

%Bacteroidetes HFHS
NN_bacter = length(tax_hf);
KK_bacter = length(bacter_inds_hf);
nn_bacter = length(out_inds_hf);
kk_bacter = length(bacter_out_inds_hf);
pval_hygeo_bacter_hf = hygecdf(kk_bacter,NN_bacter,KK_bacter,nn_bacter,'upper');

%Firmicutes HFHS
NN_firmi = length(tax_hf);
KK_firmi = length(firmi_inds_hf);
nn_firmi = length(out_inds_hf);
kk_firmi = length(firmi_out_inds_hf);
pval_hygeo_firmi_hf = hygecdf(kk_firmi,NN_firmi,KK_firmi,nn_firmi,'upper');
